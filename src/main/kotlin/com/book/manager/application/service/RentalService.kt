...
class RentalService(
  ...
  fun startRental(bookId: Long, userId: Long) {
    ...
    val book = bookRepository.findWithRental(bookId) ?: throw IllegalArgumentException(
      "該当する書籍が存在しません。bookId:$bookId")
  ...
  fun endRental(bookId: Long, userId: Long) {
    ...
    if (book.rental!!.userId != userId) throw IllegalStateException(
      "他のユーザーが貸出中です。userId:$userId, bookId:$bookId")
